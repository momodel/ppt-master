...
设计合同（`design_spec.md` + `spec_lock.md`）由 skill 冻结模板库 `templates/locked/` 直接定死；选择与落盘规则见 [`locked-template-contract.md`](./locked-template-contract.md)。大模型不再设计模式和视觉风格、HEX、字体栈、字号、留白、布局与页面节奏；视觉身份以所选模板的冻结核同为准，禁止修改。

大模型自主决定并锁定：

- 从冻结模板库选择哪套模板（按 `subjects` 匹配，回退 keji-jiangjie-v1）；
- 内容如何从资料展开到每页；
- 每页 `layout` 从模板 `pageTypes` 中选哪个，并映射到布局母版；
...
应用跳过八项视觉确认不等于默认使用某套模板。当教师确认稿和课程资料没有明确指定视觉模板时：

- 必须先读取 `templates/locked/` 全部模板的 `meta.json`，按 [`locked-template-contract.md`](./locked-template-contract.md) 以 `subjects` 标签命中最多者胜出；平局按固定顺序，无命中回退 `keji-jiangjie-v1`。
- “教学”“课程”“培训”“亲和”本身不是选择或更换某套模板的理由；选择依据只能是学科／场景标签与大纲主题的匹配。
...
- 模板落盘前必须核对：所选模板 `spec_lock.md ## layout - variation` 的构图家族分配与按页数适配后的节奏是否满足上述要求；不满足时改选其他模板，禁止改写模板合同。
