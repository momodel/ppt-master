...
> **Application override:** 本次生成固定不使用任何图片。Strategist 直接选择 **A**（No images），`design_spec.md` 不写图片资源列表，`spec_lock.md` 不得包含 `## images` section，`deck_manifest.json` 不得包含图片字段。禁止调用 `analyze_images.py`，禁止读取图片分析文件，禁止 AI、Web、切片、教师资料原图、远程 URL、data URI 或虚构文件。
...
**Application override:** 本次生成固定选择 **A**（No images），不使用任何图片，跳过图片分析与资源列表。
