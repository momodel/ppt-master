...
Only teacher-provided images already downloaded by the application may be used. The complete contract is in [`svg-image-embedding.md`](svg-image-embedding.md).

- The image must exist in `analysis/source_images.json` with `status: downloaded`.
- The same file must be locked in `spec_lock.md ## images`.
- Reference it only as `../images/<real filename>`.
- Entries ending in `| no-crop` use `preserveAspectRatio="xMidYMid meet"`; other teacher photos may use `xMidYMid slice` when the teaching subject remains visible.
- If no suitable image is locked, construct the page with native SVG, real icons, typography, and indexed charts. Do not create placeholders or request another image.
- Remote URLs, data URIs, generated images, web-sourced images, sliced images, formula images, and template-bundled bitmaps are forbidden.
